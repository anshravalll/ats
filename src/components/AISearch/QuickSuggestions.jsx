...
const quickSuggestions = [
  {
    ...
    query: "Show me React developers located in Germany with 3+ years experience"
  },
  {
    icon: <Award className="w-4 h-4" />,
    text: "Senior candidates available now",
    query: "Find senior developers with 7+ years experience who are available immediately"
  },
  {
  ...
  },
  {
    ...
    query: "Find full-stack developers who know Python and have database experience"
  }
];
...
const QuickSuggestions = ({ onSuggestionClick }) => {
  ...
  return (
    <div className="mt-4 space-y-2">
      <p className="text-xs text-muted-foreground font-medium">Quick searches:</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {quickSuggestions.map((suggestion, index) => (
          ...
          >
            <div className={`transition-colors ${hoveredIndex === index ? 'text-primary' : 'text-muted-foreground'}`}>
              {suggestion.icon}
            </div>
