...
export function useMCPWorkflow() {
  ...
  const executeMCPWorkflow = useCallback(
    async (userMessage, candidates, applyAIFilters) => {
      if (!userMessage.trim() || isProcessingMCP || candidates.length === 0)
        return;
      ...
      try {
        ...
        const { filterCandidates, rankCandidates } = await import(
          "../lib/mcp-tools"
        );

        // Filter candidates based on THINK output
        const filteredCandidates = filterCandidates(
          thinkResult.data.filter,
          candidates,
        );
  ...
  const getPhaseMessage = useCallback(() => {
    switch (currentPhase) {
      ...
      case "ACT":
        return "🔍 Filtering and ranking candidates...";
      case "SPEAK":
