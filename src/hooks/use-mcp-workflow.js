...
export function useMCPWorkflow() {
  ...
  const {
    ...
    onFinish: (message) => {
      ...
      console.log('✅ Final SPEAK response displayed');
    }
  });
  ...
  const executeMCPWorkflow = useCallback(async (userMessage, candidates, applyAIFilters) => {
    ...
    try {
      ...
      if (!csvSent) {
        setCsvSent(true);
        console.log('📤 CSV data sent with first request');
      }
      ...
      console.log('✅ THINK completed:', thinkResult.data);
      ...
      console.log('✅ ACT completed:', topCandidates.length, 'top candidates');

      // PHASE 3: SPEAK (Will be visible to user)
      setCurrentPhase('SPEAK');
      console.log('💬 Starting SPEAK phase...');
