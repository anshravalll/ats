...
export function rankCandidates(ids, plan, candidates) {
  if (!plan || typeof plan !== "object") {
    console.warn(
      "Invalid rank plan, returning candidates filtered by ids only",
    );
...
export function aggregateStats(ids, candidates) {
  ...
  const totalExperience = subset.reduce(
    (sum, c) => sum + (parseFloat(c.experience) || 0),
    0,
  );
  const avg_experience =
    count > 0 ? Math.round((totalExperience / count) * 10) / 10 : 0;
...
function getFieldValue(candidate, field) {
  ...
  if (
    ...
    field === "age" ||
    field === "desired_salary_usd" ||
    field === "remote_experience_years"
  ) {
